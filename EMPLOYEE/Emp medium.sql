...
AND S.SALARY = J.MINIMUM_SALARY;

WITH MIN_SALARY AS (SELECT JOB_TITLE ,MIN(SALARY) AS MIN_SAL FROM SALARIES GROUP BY JOB_TITLE)
SELECT S.*
FROM SALARIES S
JOIN MIN_SALARY J
ON S.JOB_TITLE = J.JOB_TITLE
AND S.SALARY = J.MIN_SAL;

/** 1. THIRD HIGHEST SALARY **/

WITH SALARY_RAN AS (SELECT SALARY,JOB_TITLE,DENSE_RANK() OVER (ORDER BY SALARY DESC) AS RAN
FROM SALARIES) 
SELECT JOB_TITLE,SALARY FROM SALARY_RAN WHERE RAN = 3;

SELECT DISTINCT SALARY FROM SALARIES
ORDER BY SALARY DESC
LIMIT 1 OFFSET 2;

SELECT DISTINCT SALARY ,JOB_TITLE FROM SALARIES 
ORDER BY SALARY DESC
LIMIT 1 OFFSET 2;
...
SELECT DEL_PARTNER ,SUM(CASE WHEN TIMESTAMPDIFF(MINUTE,ORDER_TIME,DELIVER_TIME)>PREDICTED_TIME THEN 1 ELSE 0 END) AS TOTAL_DELAY 
FROM SWIGGY_ORDERS
GROUP BY DEL_PARTNER
...
SELECT ORDER_DATE , SUM(ORDER_VALUE) AS DAILY_SALES FROM SWIGGY_SALES WHERE PRODUCT_ID = 1
...
SELECT ORDER_DATE , SUM(ORDER_VALUE) AS DAILY_SALE FROM SWIGGY_SALES WHERE PRODUCT_ID = 1
GROUP BY ORDER_DATE
...
SELECT * FROM SALARIES WHERE SALARY =(SELECT DISTINCT(SALARY) AS SALARY FROM SALARIES 
ORDER BY SALARY DESC
LIMIT 1 OFFSET 1);

WITH RANKING AS (SELECT * , DENSE_RANK() OVER (ORDER BY SALARY DESC) AS RNK FROM SALARIES)
SELECT * FROM RANKING WHERE RNK = 2;
-- THIS CAN BE USED BUT IT CAN GIVE ONE ROW ONLY BUT IN OUR ANS THERE ARE MORE THAN 1 SO CAN NOT USE THIS 
SELECT * FROM SALARIES S2 WHERE 2 = (SELECT DISTINCT (S1.SALARY) FROM SALARIES S1 WHERE S1.SALARY >= S2.SALARY); 


-- FIND WHO EARN MORE THAN DEPT AVG SALARY
SELECT * FROM SALARIES S1 WHERE SALARY > (SELECT AVG(SALARY) AS AVERG_SAL FROM SALARIES WHERE JOB_TITLE = S1.JOB_TITLE);

SELECT * FROM WORKER;
-- FIND THE OLDEST EMPLOYEE
 SELECT * FROM WORKER W WHERE JOINING_DATE = (SELECT MIN(JOINING_DATE) FROM WORKER WHERE W.DEPARTMENT = DEPARTMENT);
 
 Select * from worker;
-- salaries of those joined after 2014-02-20 
